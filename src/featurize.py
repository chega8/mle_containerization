...
def fearurize(prepared_path: str, output_dir: str):
    ...
    TARGET = 'target'
    FEATURES = [col for col in prepared_train.columns if col not in ['id', TARGET]][:MAX_FEATURES]
    
    # Transform train and test features
    for col in FEATURES:
        prepared_train[col] = scaler.fit_transform(prepared_train[col].to_numpy().reshape(-1, 1))
        prepared_test[col] = scaler.transform(prepared_test[col].to_numpy().reshape(-1, 1))
